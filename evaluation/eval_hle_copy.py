...
import sys
from LLM_API import get_llm_response, MODEL_MAPPING, tavily_client
...
def call_tool(arguments):
    ...
    if arguments['tool']=='enhance_reasoning':
        supported_models = ALL_TOOLS['enhance_reasoning'] ['model']
        assert arguments['model_alias'] in supported_models,f"Model {arguments['model_alias']} is not supported in enhance_reasoning. Support models: {supported_models}"
        ...
        model_name = arguments['model_alias']
        response = ''
        ...
        try:
            ...
            # ===== 调试日志：打印代码执行输出 =====
            print(f"\n🔧 [enhance_reasoning] 代码执行结果:")
            print(f"   输出长度: {len(exec_result)} 字符")
            if exec_result:
                # 打印前500字符（避免输出过长）
                preview = exec_result[:500]
                print(f"   输出预览:\n{preview}")
                if len(exec_result) > 500:
                    print(f"   ... (还有 {len(exec_result) - 500} 字符)")
            else:
                print(f"   ⚠️ 无输出或执行失败")
            print("=" * 70)
        ...
        if pred.strip()=='' or len(pred.split(' '))>500:
            ...
            eval_response = get_llm_response(
                model_alias='eval-1',
                messages=[{"role": "user", "content": eval_prompt}],  # ✅ 改成列表格式
                temperature=1
            )
            eval_result = eval_response.choices[0].message.content.split('<correct>')[-1].split('</correct>')[0]  # ✅ 正确提取响应
        ...
        cur_query_writer = arguments['model_alias']
        query_to_call = None
        try:
            response = get_llm_response(
            ...
            # 提取 query 标签内容
                ...
                if '<query>' in content and '</query>' in content:
                    query_to_call = content.split('<query>')[-1].split('</query>')[0].strip()
                else:
                    query_to_call = arguments['problem']
        except Exception as e:
        ...
        if query_to_call and len(query_to_call) >= 5:
            ...
            try:
                ...
                if search_results and 'results' in search_results:
                    ...
                    # 打印前3个结果的相关性分数（用于调试）
                    print(f"\n📊 Tavily返回 {len(sorted_results)} 个结果，按相关性排序:")
                    for idx, result in enumerate(sorted_results[:3]):
                        score = result.get('score', 0)
                        url = result.get('url', 'N/A')
                        print(f"  #{idx+1}: score={score:.3f}, url={url[:60]}...")
...
def run_single(e):
    ...
    for step in range(MAX_ROUNDS):
        ...
        for one_tool_call in tool_calls:
            ...
            expert_model_to_call = MODEL_MAPPING[tool_arguments['model']]['model']
            ...
            if tool_name=='enhance_reasoning':
                ...
                elif 'deepseek-chat' in expert_model_to_call.lower():
                ...
                call_tool_argument = {
                    'tool': tool_name,
                    'model_alias': tool_arguments['model'],
                ...
                if 'Qwen/Qwen2.5-72B-Instruct' in expert_model_to_call.lower():
                    ...
                    max_code_length = 20000
                    max_context_length = 30000
                elif 'Qwen/Qwen3-30B-A3B' in expert_model_to_call.lower():
                    max_code_length = 16000
                ...
                doc_str = ''
                for doc_idx, doc in enumerate(doc_list):
                    if 'Pro/zai-org/GLM-4.7' in expert_model_to_call.lower():
                        doc_str += f"Doc {doc_idx+1}: {doc}\n\n"
                    else:
                        doc_str += f"Doc {doc_idx+1}: {doc[:1000]}\n\n"
                code_str = ''
                ...
                if 'Qwen/Qwen3-Next-80B-A3B-Thinking' in expert_model_to_call.lower():
                    max_code_length = 12000
                    max_context_length = 24000
                elif 'Pro/zai-org/GLM-4.7' in expert_model_to_call.lower():
                    max_code_length = 40000
        ...
        for cur_response in cur_responses:
            if cur_response['tool']=='enhance_reasoning':
                ...
                final_answer_model = cur_response['model_alias']
                final_pred = cur_response['pred'].strip()
                finish_flag = True
                break
            elif cur_response['tool']=='search':
                # 按原顺序添加（高分文档在前，已在search工具中排序）
                for one_doc in cur_response['search_results_data']:
