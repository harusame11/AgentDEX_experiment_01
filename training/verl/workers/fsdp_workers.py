...
class ActorRolloutRefWorker(Worker):
    ...
    def __init__(self, config: DictConfig, role: str):
        ...
        # normalize config
        if self._is_actor:
            print(145,'self.config.rollout.n',self.config.rollout.n)
            print(145,'self.config.actor.ppo_mini_batch_size',self.config.actor.ppo_mini_batch_size,'self.device_mesh.size()',self.device_mesh.size(),'self.ulysses_sequence_parallel_size',self.ulysses_sequence_parallel_size)
            self.config.actor.ppo_mini_batch_size *= self.config.rollout.n
            print(147,'self.config.actor.ppo_mini_batch_size',self.config.actor.ppo_mini_batch_size,'self.device_mesh.size()',self.device_mesh.size(),'self.ulysses_sequence_parallel_size',self.ulysses_sequence_parallel_size)
            self.config.actor.ppo_mini_batch_size //= self.device_mesh.size() // self.ulysses_sequence_parallel_size
            print(149,'self.config.actor.ppo_mini_batch_size',self.config.actor.ppo_mini_batch_size,'self.device_mesh.size()',self.device_mesh.size(),'self.ulysses_sequence_parallel_size',self.ulysses_sequence_parallel_size)
    ...
    def generate_sequences(self, prompts: DataProto):
        # Support all hardwares
        print(615,"prompts.batch['input_ids'].shape",prompts.batch['input_ids'].shape)
        prompts = prompts.to(get_torch_device().current_device())
        # print(616,'get_torch_device()',get_torch_device())
        # print(617,'get_torch_device().current_device()',get_torch_device().current_device())
        import socket
        hostname = socket.gethostname()
        IPAddr = socket.gethostbyname(hostname)
        print("fsdp worker 622 Your Computer Name is:" + hostname)
        print("fsdp worker 623 Your Computer IP Address is:" + IPAddr)
        ...
        with self.rollout_sharding_manager:
            ...
            # print(636,"prompts.batch['input_ids'].shape",prompts.batch['input_ids'].shape)
            prompts = self.rollout_sharding_manager.preprocess_data(prompts)
            # print(638,"prompts.batch['input_ids'].shape",prompts.batch['input_ids'].shape)
            with _timer("generate_sequences", timing_generate):
                output = self.rollout.generate_sequences(prompts=prompts)
            # print(641,"output.batch['input_ids'].shape",output.batch['input_ids'].shape)
