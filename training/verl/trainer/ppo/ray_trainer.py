...
from verl.nvidia.utils.timer import TimeoutChecker
...
class RayPPOTrainer:
    ...
    def _create_dataloader(self, train_dataset, val_dataset, collate_fn, train_sampler):
        ...
        # print(482,'self.config.data.train_files',self.config.data.train_files)
        # print(487,'self.config.data.val_files',self.config.data.val_files)
    ...
    def _load_checkpoint(self):
        print(828,'self.config.trainer.resume_mode',self.config.trainer.resume_mode)
        ...
        print(841,'global_step_folder',global_step_folder,flush=True)

        if self.config.trainer.resume_mode == "auto" and global_step_folder is None:
            print("844 Training from scratch",flush=True)
            return 0

        # find global_step_folder
        if self.config.trainer.resume_mode == "auto":
            if global_step_folder is None:
                print("850 Training from scratch")
                return 0
        else:
        ...
        # load actor
        print(866,'before loading actor')
        self.actor_rollout_wg.load_checkpoint(actor_path, del_local_after_load=self.config.trainer.del_local_ckpt_after_load)
        print(868,'after loading actor')
        # load critic
        print(870,'before loading critic',self.use_critic)
        if self.use_critic:
            self.critic_wg.load_checkpoint(critic_path, del_local_after_load=self.config.trainer.del_local_ckpt_after_load)
        print(873,'after loading critic',self.use_critic)

        # load dataloader,
        # TODO: from remote not implemented yet
        print(877,'before loading dataloader')
        ...
        print(887,'after loading dataloader')
