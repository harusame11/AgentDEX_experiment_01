            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("gen", timing_raw):
                        print(112,gen_batch.batch.keys())
                        print(113,gen_batch.batch['input_ids'].shape)
                        gen_batch_output = self.actor_rollout_wg.generate_sequences(gen_batch)
                        print(115,gen_batch_output.batch.keys())
                        for k,v in gen_batch_output.batch.items():
                            try:
                                print(118,k,v.shape)
                            except:
                                print(118,key,'non-tensor')
                        print(121,gen_batch_output.non_tensor_batch.keys())
                    ...
                    with _timer("reward", timing_raw):
                        ...
                        # rule-based rm
                        reward_result = ray.get(self.reward_fn.__call__.remote(new_batch))
                        reward_extra_infos_dict = {}

                        new_batch.batch["token_level_scores"] = reward_result["score"]
                        new_batch.batch['token_level_rewards'] = reward_result['reward']
